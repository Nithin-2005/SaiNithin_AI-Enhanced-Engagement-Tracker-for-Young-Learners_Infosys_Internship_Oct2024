...
if not os.path.exists("ssofave_attention_score"):
    os.makedirs("ssofave_attention_score")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"ssofave_attention_score/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
    ...
    if not df.empty:
        ...
        df.to_excel('attendance_with_attention_and_average nov 6.xlsx', index=False)
