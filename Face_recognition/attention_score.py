...
# Create a directory to save ssofattention_score
if not os.path.exists("ssofattention_score"):
    os.makedirs("ssofattention_score")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"ssofattention_score/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
    ...
    df.to_excel('attendance_with_attention_nov_6.xlsx', index=False)
